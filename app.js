...
function add(id) {
  if (id !== app.querySelector(".bottom").getElementsByTagName("button")[0]) {
    ...
    if (id.parentElement === app.querySelector(".number")) test = true;
    else test = false;

    if (id.parentElement === app.querySelector(".sing")) point = true;
  } else if (point) {
    input.value += id.textContent;
    point = false;
  }
...
function boolee(n) {
  for (let i = 0; i < sing.length; i++) {
    if (sing[i].textContent === value[n]) return true;
  }
  return false;
}
function start() {
...
function A(i) {
  if (i < value.length) {
    ...
    for (let i = 0; i < sorting.length; i++) {
      if (test(sorting[i])) sorting[i] = Number(sorting[i]);
    }
    end();
  }
}
...
function where(sing) {
  whereSing = [];
  for (let i = 0; i < sorting.length; i++) {
    if (sing === sorting[i]) whereSing.push(i);
  }
}
function end() {
  where("*");
